...
- 管理后台去卡片化：移除「环境检测 / DeepSeek Harness / 服务器」三张卡片，页面精简为「logo+标题 → 状态总览条（Node / Harness / 服务器，颜色区分状态）→ 服务器操作工具栏（启动/停止/重启/应用内打开/在浏览器打开/复制地址）→ 运行日志 → 关于」；Node/Harness 安装、升级、切换、校验统一由「关于」区新增的「重新运行安装引导」入口承接（引导流程自动检测补齐）
- 入口文案统一：「返回鲸仔助手」「进入高级面板」等全部改为「进入鲸仔管理后台」；管理后台顶部加入鲸仔 logo 与「鲸仔管理后台」标题，日志卡片支持手动刷新并显示条数，页脚新增关于区（鲸仔版本 / GitHub 链接 / 隐藏到托盘）
