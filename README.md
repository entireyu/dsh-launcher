...
1. 在 [Releases](https://github.com/entireyu/dsh-whalito-desk/releases) 下载最新安装包 `Whalito_0.4.0_x64-setup.exe`
...
1. 在 [Releases](https://github.com/entireyu/dsh-whalito-desk/releases) 下载最新 `Whalito_0.4.0_universal.dmg`
...
```bash
pnpm tauri:build        # 生产包：Whalito_0.4.0_x64-setup.exe（DSH 端口 3080，数据目录 ~/.dsh）
pnpm tauri:build:test   # 测试包：Whalito-Test_0.4.0_x64-setup.exe（DSH 端口 30080，数据目录 ~/.dsh-test）
```
...
- 打 `v*` tag 或手动触发 Release workflow → CI 同时构建 Windows NSIS 与 macOS universal dmg，并上传到 GitHub Release（资产名如 `Whalito_0.4.0_universal.dmg`）
